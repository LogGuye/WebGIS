...
def _apply_property_filters(request, queryset):
    from django.contrib.gis.measure import D  # Import thư viện tính khoảng cách PostGIS
    ...
    # BẮT THÊM THÔNG SỐ BÁN KÍNH TỪ FORM
    ...
    # LỌC THEO VÒNG TRÒN BÁN KÍNH (SPATIAL QUERY)
    if lat and lng and radius:
        try:
            pnt = Point(float(lng), float(lat), srid=4326)
            queryset = queryset.filter(location__distance_lte=(pnt, D(km=float(radius))))
        except (TypeError, ValueError):
            pass
